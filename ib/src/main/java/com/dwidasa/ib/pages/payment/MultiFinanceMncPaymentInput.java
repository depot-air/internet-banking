 ...
 * Time: 22:43
 */
public class MultiFinanceMncPaymentInput {
    @Persist
    @Property
    ...
    @InjectPage
    private MultiFinanceMncPaymentConfirm multiFinancePaymentConfirm;
    ...
    public void setupRender() {
    	if (!sessionManager.getSessionLastPage().equals(MultiFinanceMncPaymentInput.class.toString()) ) {
    		multiFinancePaymentView = null;
    	}
    	sessionManager.setSessionLastPage(MultiFinanceMncPaymentInput.class.toString());
    ...
    void onValidateFromForm() {
        try {
            if (!form.getHasErrors() ) {
                setPaymentView();
                multiFinancePaymentView.setTransactionType(Constants.MULTIFINANCE_NEW.INQ_MULTI_FINANCE_MNC);
