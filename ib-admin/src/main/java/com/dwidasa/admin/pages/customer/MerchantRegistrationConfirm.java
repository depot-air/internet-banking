...
public class MerchantRegistrationConfirm {
	private static Logger logger = Logger.getLogger( MerchantRegistrationConfirm.class );
    ...
    @InjectPage
    private MerchantRegistrationReceipt merchantRegistrationReceipt;
    ...
    public Object onSuccess() {
    	merchantRegistrationReceipt.setCustomer(customer);
    	merchantRegistrationReceipt.setCustomerAccount(customerAccount);
    	merchantRegistrationReceipt.setCustomerDevice(ibsCustomerDevice);
        return merchantRegistrationReceipt;
    }
    ...
    void onValidateFromForm() {	
    	...
    	List<CustomerDevice> customerDevices = null;
    	List<CustomerDevice> customerDevicesActive = null;
    	
    		try {
				
    			customerDevices = customerDeviceService.getNotActiveByCustomerId(customer.getId());
    		...
    		try {
    			customerDevicesActive = customerDeviceService.getDeviceActive(customer.getId());
			} catch (Exception e) {
				// TODO: handle exception
			}
        	...
        	if(saveBoxValue){
        		...
        		if(customerDevicesActive.size() <= 0){
            		form.recordError("Data Device Belum Ada Untuk Customer Ini");
            	}
        		
        	}else{
        		
        		if(customerDevices.size() <= 0){
            		form.recordError("Data Device Belum Ada Untuk Customer Ini");
            	}
    		...
    		for(int i=0; i<ibTokens.size(); i++) {
        		...
        		if (ibt.getSerialNumber().equals(ibToken.getSerialNumber())) {
        			...
        			if(saveBoxValue){
        				...
        				if (customerDevicesActive != null && customerDevicesActive.size() > 0) {
                			try {
            	            	customer = customerService.getByUsername(userIdValue.toUpperCase());
              
            	            		System.out.println("updating  m_customer_device");
    								ibsCustomerDevice = customerDevicesActive.get(0);
    								ibsCustomerDevice.setTerminalId(merchantIdValue.toString());
    								ibsCustomerDevice.setDeviceId(customerDevicesActive.get(0).getDeviceId());
        			...
        			if (customerDevices != null && customerDevices.size() > 0) {
            			try {
        	            	customer = customerService.getByUsername(userIdValue.toUpperCase());
								...
								String result = tokenAgentService.addCustomerRetailRegistration(
												customer.getCustomerUsername(),
												customer.getCustomerName(),
												customer.getCifNumber(),
												ibToken.getSerialNumber());
...
//        	stmt.executeUpdate(qInsert);
    ...
    Object onSelectedFromReset() {
        ...
        return MerchantRegistrationInput.class;
    }
